...
public class KlavaOrTest extends ClientServerBase {
	...
	/**
	 * Abstract base class for test processes that participate in an OR.
	 * Tracks whether the process successfully completed its retrieval.
	 */
	...
	/**
	 * A process that calls the OR operator on a list of processes.
	 * This is needed because the or() method must be called from within a process
	 * execution context (which provides the klavaNodeProcessProxy).
	 */
	...
	/**
	 * Test scenario: exactly one tuple matching only one branch's template.
	 * The matching branch should succeed; the non-matching branch should be
	 * interrupted and terminated.
	 */
	public void testOrOneSucceeds() throws InterruptedException, IMCException,
			KlavaException {
	...
	/**
	 * Test scenario: two identical tuples in the tuple space; two in-processes
	 * with the same template race to retrieve them. Exactly one should succeed,
	 * and the tuple space should end up with exactly one tuple (the loser
	 * re-inserts the one it retrieved).
	 */
	public void testOrBothCouldSucceedInRace() throws InterruptedException,
		...
		int successCount = (branchA.succeeded ? 1 : 0)
				+ (branchB.succeeded ? 1 : 0);
		...
		assertTrue("Should have a tuple in tuple space",
				clientNode.in_nb(template, self));
		assertFalse("Should not have a second tuple",
				clientNode.in_nb(template, self));
	}

	/**
	 * Test scenario: one tuple in the tuple space; two read-processes with the
	 * same template. Only one should have succeeded=true; the other should have
	 * failed. The tuple should remain in the tuple space (because read is
	 * non-destructive).
	 */
	public void testOrWithRead() throws InterruptedException, IMCException,
			KlavaException {
		// Put one tuple
		...
		Tuple template = new Tuple(new KString());
		template.setHandleRetrieved(false);
		assertTrue("Tuple should remain in tuple space",
				clientNode.in_nb(template, self));
	}

	/**
	 * Test scenario: tuple matches only branch B's template, not A's. Branch A
	 * will block waiting for a non-existent tuple; branch B will immediately
	 * succeed.
	 */
	public void testOrFirstFails() throws InterruptedException, IMCException,
			KlavaException {
	 ...
	 */
	public void testOrThreeProcesses() throws InterruptedException,
			IMCException, KlavaException {
		// Put one tuple
		...
		// Exactly one should have succeeded
		int successCount = (branchA.succeeded ? 1 : 0)
				+ (branchB.succeeded ? 1 : 0) + (branchC.succeeded ? 1 : 0);
		assertEquals("Exactly one branch should succeed", 1, successCount);
	 ...
	 * without blocking.
	 */
	public void testOrWithInNb() throws InterruptedException, IMCException,
			KlavaException {
	...
	/**
	 * Test scenario: KlavaOrProcess can be used standalone (not in an OR) without
	 * a mutex. A simple in operation should work normally.
	 */
	public void testKlavaOrProcessStandalone() throws InterruptedException,
			IMCException, KlavaException {
		// Put a tuple
		...
		assertFalse("Tuple should be removed",
				clientNode.in_nb(template, self));
	}
}
