 ...
 * 
 * - Exactly one process in the OR succeeds with its retrieval operation - All
 * other processes are interrupted - If a non-first process removed a tuple (in,
 * in_nb, in_t), it re-inserts the tuple before terminating
 * 
...
public abstract class KlavaOrProcess extends KlavaProcess {
    ...
    /**
     * Handles the result of an OR retrieval operation. This method is called by the
     * overridden retrieval methods after a successful retrieval.
     * 
     ...
     * @param destination    the locality where the retrieval was performed
     * @param isRemoval      true if the retrieval was a removal operation (in,
     *                       in_nb, in_t), false if it was a read operation (read,
     *                       read_nb, read_t)
     * @throws KlavaException if this process lost the race (not the first to
     *                        succeed)
     */
    private void handleOrRetrieval(Tuple retrievedTuple, Locality destination, boolean isRemoval)
    ...
    @Override
    protected boolean in_nb(Tuple tuple, Locality destination) throws KlavaException {
    ...
    @Override
    protected boolean read_nb(Tuple tuple, Locality destination) throws KlavaException {
    ...
    @Override
    protected boolean in_t(Tuple tuple, Locality destination, long timeout) throws KlavaException {
    ...
    @Override
    protected boolean read_t(Tuple tuple, Locality destination, long timeout) throws KlavaException {
